...
# An input field with no 'name' attribute prevents it from being passed to
# the server
...
class PaymillForm(forms.Form):
    cardnumber = forms.CharField(
        ...
        max_length=16,
        widget=NameLess(attrs={
            'class': 'card-number rem',
            'size': '16',
            'placeholder': '**** **** **** ****',
        })
    )
    ...
    cvc = forms.CharField(
        ...
        max_length=3,
        widget=NameLess(
            attrs={'class': 'card-cvc rem', 'size': '3', 'placeholder': '***'})
    )
